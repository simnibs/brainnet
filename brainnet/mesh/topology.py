...
# def get_recursively_subdivided_topologymodule(faces: torch.IntTensor | None = None, n_recursions: int = 0):

#     assert n_recursions >= 0
#     faces = faces if faces is not None else initial_faces

#     topology = TopologyModule(faces)
#     topologies = [topology]
#     for _ in range(0, n_recursions):
#         topology = TopologyModule(topology.subdivide_faces())
...
# # DUPLICATE OF TOPOLOGY!!!
# class TopologyModule(torch.nn.Module):
...
#         assert self.n_vertices == features.shape[-1], f"Expected {self.n_vertices} to unpool from (got {features.shape[-1]})."

#         up_features = features[..., self.vertex_adjacency]
#         up_features = getattr(up_features, reduce)(-1) # apply method from string
...
#         assert self.n_vertices == features.shape[-1], f"Expected {self.n_vertices} to pool from (got {features.shape[-1]})."
...
#         pooled = torch.empty_like(own_features).copy_(own_features)
#         pooled.index_reduce_(
#             -1, reduce_index, features[..., gather_index], reduce=reduce, include_self=True
#         )
...
#         faces_enum = torch.arange(
#             self.n_faces, dtype=dtype, device=self.faces.device
#         )[:, None].expand_as(self.faces).ravel()
#         face_adjacency = faces_enum[a0[a1[a2]]].reshape(-1,2)

#         self.register_buffer("vertex_adjacency", vertex_adjacency, persistent=False)
...
class Topology:
    ...
    def reverse_face_orientation(self):
        new_order = (0, 2, 1)
        self.faces = self.faces[:, new_order]
        # Ensure subdivide_faces is still valid
        if hasattr(self, "faces_to_edges"):
            self.faces_to_edges = self.faces_to_edges[:, new_order]
