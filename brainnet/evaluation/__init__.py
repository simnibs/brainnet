...
def create_evaluator(
    ...
    eval_step = model_helper.EvaluationStep(
        synth, model, criterion, enable_amp=setup.enable_amp
    )
...
def evaluate(
    ...
    model = "topofit"
    specs_name = "synth_random_adnigo2"
    subset = "validation"
    checkpoint = 800
...
def predict(
    ...
    for dataset, subject in content:
        ...
        batch = ds[i]
        images = dict(image=batch[0][contrast])
        vox2ras = dict(image=batch[1][contrast])
