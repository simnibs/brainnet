...
class AffineCorticalAlignment(torch.nn.Module):
    def __init__(
        ...
        self.image_fx_pre = torch.nn.Conv3d(1, 8, kernel_size=3, stride=2, padding=1)
        ...
        self.image_fx_post = torch.nn.Conv3d(32, self.ppb, kernel_size=3, padding=1)

        # Template points. Learns the coordinates (in template world space)
        # corresponding to the salient regions extracted from the image of
        # particular subject
    ...
    def forward(self, image, vox_to_mri, return_affine_brain: bool = True):
        ...
        if self.image_shape != (spatial_shape := features.shape[-3:]):
            self.image_grid = torch.stack(
                torch.meshgrid(
                    [torch.arange(s, device=self.device) for s in spatial_shape],
                    indexing="ij",
                )
        ...
        if self.weigh_by_feature_mass:
            feature_mass = features.sum(self.spatial_dims, keepdim=True) + 1e-6
        ...
        if return_affine_brain:
            return affines | dict(brain=self.estimate_affine_brain(target))
        else:
            return affines
