...
class BrainNet(torch.nn.Module):
    def __init__(self, body: torch.nn.Module, heads: dict[str, torch.nn.Module], device: str | torch.device):
        ...
        #     {
        #         name: getattr(head, brainnet.modules.head.module.name)(**vars(brainnet.modules.head.module.kwargs))
        #         for name, head in vars(head_config).items()
        #     }
        # )

        biasfieldmodule = [h for h in self.heads.values() if isinstance(h, brainnet.modules.head.BiasFieldModule)]
